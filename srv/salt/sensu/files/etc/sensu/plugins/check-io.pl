...
if ($help || $diskname eq ""){
  ...
  logger("   -d = physical disk to check", 0);
  logger("   -v = run in debug mode",0);
...
if (-d eq "swap" ){
  ...
  while(<SWAP>){
    ...
    if ($filename=~/mapper/){
      ...
      logger("85 Found swap device: $filename", 1);
      $filename=~s/\/dev\///g;
      $dev = $filename;
    }
  }
...
sub getOps(){
  ...
  logger("\tRetrieving Disk data from $diskstats", 1);
  my $grep_cmd = `grep $r_dev $diskstats | head -1`;
  logger("\tcommand: grep $r_dev $diskstats| head -1",1);
  chomp $grep_cmd;
